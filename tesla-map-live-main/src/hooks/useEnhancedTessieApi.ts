...
export const useEnhancedTessieApi = (apiKey?: string) => {
  ...
  const fetchVehicleData = useCallback(async (vehicleId: string) => {
    try {
      setError(null);
      const data = await makeApiCall(`/vehicles/${vehicleId}/state`);
      
      const vehicleData: VehicleData = {
        battery_level: data.charge_state?.battery_level || 0,
        battery_range: data.charge_state?.battery_range || 0,
        charging_state: data.charge_state?.charging_state || 'Unknown',
        ...
        timestamp: data.drive_state?.timestamp || Date.now(),
      };
