 ...
 */
router.get('/dispatch/functions', async (request, env) => {
 ...
 * Execute a function
 */
router.post('/dispatch/functions/:name', async (request, env, context) => {
    // Check authentication
    const authResult = await requireAuth(request, env);
    if (authResult instanceof Response) {
        return authResult;
 ...
 */
router.put('/dispatch/deploy', async (request, env) => {
    // Check authentication
    const authResult = await requireAuth(request, env);
    if (authResult instanceof Response) {
        return authResult;
