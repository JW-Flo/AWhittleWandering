 ...
 */
router.get('/api/trip/current', async (request, env) => {
    try {
 ...
 */
router.get('/api/trip/day/:day', async (request, env, context) => {
    try {
 ...
 */
router.post('/api/trip/telemetry', async (request, env) => {
 ...
 */
router.get('/api/summary/:day', async (request, env, context) => {
    try {
        const day = parseInt(context.params.day, 10);
        if (isNaN(day) || day < 1 || day > 60) {
            return jsonResponse({
        ...
        ${itineraryData ?
                `Planned stops: ${itineraryData.stops?.map((stop) => stop.location).join(', ')}` :
