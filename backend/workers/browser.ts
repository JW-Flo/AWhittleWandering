 ...
 */
function binaryResponse(data: ArrayBuffer, contentType: string, filename?: string) {
 ...
 */
router.get('/render/screenshot', async (request: Request, env: Env) => {
  ...
  try {
    ...
    const format = (url.searchParams.get('format') || OutputFormat.PNG) as OutputFormat;
 ...
 */
router.get('/render/pdf', async (request: Request, env: Env) => {
  // Check authentication
  const authResult = await requireAuth(request, env);
  if (authResult instanceof Response) {
    return authResult;
 ...
 * - download: Whether to download the image - default: false
 */
router.get('/render/og-image', async (request: Request, env: Env) => {
  // Check authentication
  const authResult = await requireAuth(request, env);
  if (authResult instanceof Response) {
    return authResult;
