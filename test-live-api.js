...
async function testTessieAPI() {
  ...
  try {
    ...
    if (vehicles.results.length > 0) {
      ...
      console.log('Display Name:', vehicles.results[0].last_state?.display_name);
      ...
      const drivesResponse = await fetch(`https://api.tessie.com/${vin}/drives?from=${fromDate}&to=${toDate}`, {
        headers: { 'Authorization': `Bearer ${apiKey}` }
      });
      ...
      const chargesResponse = await fetch(`https://api.tessie.com/${vin}/charges?from=${fromDate}&to=${toDate}`, {
        headers: { 'Authorization': `Bearer ${apiKey}` }
      });
