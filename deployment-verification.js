...
function checkDeployment() {
  ...
  const isProd = window.location.href.includes(
    "awhittlewandering-site.pages.dev"
  );
...
function checkMapboxToken() {
  ...
  if (window.__MAPBOX_TOKEN__) {
    ...
    console.log(
      "Window token preview:",
      window.__MAPBOX_TOKEN__.substring(0, 20) + "..."
    );
  } else {
...
function checkCSSInclusion() {
  ...
  if (!mapboxCSSFound) {
    ...
    const mapboxStyleElements = document.querySelectorAll(
...
function checkMapContainer() {
  ...
  const mapContainers = document.querySelectorAll(
    '.map-container, .mapboxgl-map, [id*="map"]'
  );
...
function verifyDeployment() {
  ...
  const overallStatus =
    tokenOK && containerOK ? "✅ DEPLOYMENT LOOKS GOOD" : "❌ ISSUES DETECTED";
  console.log("\n" + overallStatus);
