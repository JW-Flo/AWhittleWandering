...
class SiteVerifier {
  ...
  generateReport() {
    ...
    const teslaDataTest = this.results.find(
      (r) => r.name === "Development Tesla Data"
    );
    if (teslaDataTest && teslaDataTest.ok && teslaDataTest.isJson) {
      ...
      if (data.tessieStatus) {
        ...
        console.log(
          `   Connected: ${data.tessieStatus.connected ? "✅" : "❌"}`
        );
        console.log(
          `   Data Freshness: ${data.tessieStatus.dataFreshness || "unknown"}`
        );
    ...
    const localFailed = this.results.find(
      (r) => r.name === "Local Dev Server" && !r.ok
    );
    if (localFailed) {
      console.log(
        "   • Local dev server not running - run: npm run dev:frontend"
      );
    }

    const apiFailed = this.results.filter(
      (r) => r.name.includes("API") && !r.ok
    );
