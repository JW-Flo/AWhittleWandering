...
// Timing constants for test reliability across different environments
const APP_SETTLE_DELAY_MS = 8000; // Allow time for app to fetch data and settle
const UI_INTERACTION_DELAY_MS = 1500; // Delay after UI interactions to observe changes
...
async function main() {
  ...
  try {
    ...
    await step("wait-for-app-settle", async () => {
      ...
      await sleep(APP_SETTLE_DELAY_MS);
    ...
    await step("optional-ui-interactions", async () => {
      ...
      await sleep(UI_INTERACTION_DELAY_MS);
