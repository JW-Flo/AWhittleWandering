...
// Timing constants for page interactions and data loading
const APP_SETTLE_TIME_MS = 8000;  // Time to wait for app to fetch data and settle
const UI_INTERACTION_DELAY_MS = 1500;  // Time to wait after UI interactions
...
async function main() {
  ...
  try {
    ...
    await step("wait-for-app-settle", async () => {
      ...
      await sleep(APP_SETTLE_TIME_MS);
