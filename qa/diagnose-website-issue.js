  ...
  try {
    ...
    try {
      ...
      const cssResponse = await fetch(
        "https://ab99ceea.awhittlewandering-frontend.pages.dev/assets/components-Bh_1FTD-.css"
      );
      const cssWorking = cssResponse.ok;

      // Check JavaScript vendor files
      const reactVendorResponse = await fetch(
        "https://ab99ceea.awhittlewandering-frontend.pages.dev/assets/react-vendor-6s76OEBy.js"
      );
      ...
      if (assetResults.allAssetsLoading) {
        ...
        console.log(
          `   ✅ React Vendor: ${reactVendorWorking ? "Loading" : "Failed"}`
        );
      } else {
        failedPhases++;
        ...
        console.log(
          `   ${cssWorking ? "✅" : "❌"} CSS: ${
            cssWorking ? "Loading" : "Failed"
          }`
        );
        console.log(
          `   ${reactVendorWorking ? "✅" : "❌"} React Vendor: ${
            reactVendorWorking ? "Loading" : "Failed"
          }`
        );
    ...
    try {
      ...
      const teslaResponse = await fetch(
        "https://awhittlewandering-api.kd8jc7v8cd.workers.dev/api/tesla/data"
      );
      ...
      if (apiResults.health) {
        ...
        console.log(
          `   ${teslaWorking ? "✅" : "⚠️"} Tesla Data: ${
            teslaWorking ? "Available" : "Not Available"
          }`
        );
      } else {
        failedPhases++;
    ...
    try {
      ...
      const jsBundle = await fetch(
        "https://ab99ceea.awhittlewandering-frontend.pages.dev/assets/index-CcChzyz6.js"
      );
      ...
      const hasReactErrors =
        jsCode.includes("React") && jsCode.includes("error");
      ...
      const jsAnalysis = {
        ...
        analysis:
          "Bundle loads but may have runtime errors causing black screen",
      };
      ...
      if (jsBundle.ok) {
        ...
        console.log(
          `   ${hasConsoleErrors ? "⚠️" : "✅"} Console Errors: ${
            hasConsoleErrors ? "Potential issues found" : "None detected"
          }`
        );
        console.log(
          `   ${hasReactErrors ? "⚠️" : "✅"} React Errors: ${
            hasReactErrors ? "Potential issues found" : "None detected"
          }`
        );
        console.log(
          `   ${hasMapErrors ? "⚠️" : "✅"} Map Errors: ${
            hasMapErrors ? "Potential issues found" : "None detected"
          }`
        );
    ...
    if (passedPhases >= 3) {
      ...
      console.log(
        "   • Black screen suggests React component mounting failure"
      );
      console.log("\n🔧 RECOMMENDED ACTIONS:");
