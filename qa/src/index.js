...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname.match(/^\/qa\/runs\/([^\/]+)$/) && request.method === 'GET') {
      ...
      if (url.pathname.match(/^\/qa\/runs\/([^\/]+)$/) && request.method === 'PUT') {
        const runId = url.pathname.split('/')[3];
        const body = await request.json();
        ...
        return new Response(JSON.stringify({
          success,
          message: success ? 'QA run updated successfully' : 'Failed to update QA run'
        }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      }
      ...
      if (url.pathname.match(/^\/qa\/runs\/([^\/]+)\/phases$/) && request.method === 'POST') {
        const runId = url.pathname.split('/')[3];
        const body = await request.json();
      ...
      // Record test result
      if (url.pathname.match(/^\/qa\/runs\/([^\/]+)\/tests$/) && request.method === 'POST') {
        const runId = url.pathname.split('/')[3];
        const body = await request.json();
      ...
      if (url.pathname.match(/^\/qa\/runs\/([^\/]+)\/metrics$/) && request.method === 'POST') {
        const runId = url.pathname.split('/')[3];
        const body = await request.json();
      ...
      // Get configuration
      if (url.pathname.match(/^\/qa\/config\/([^\/]+)$/) && request.method === 'GET') {
      ...
      if (url.pathname.match(/^\/qa\/config\/([^\/]+)$/) && request.method === 'PUT') {
        const configKey = url.pathname.split('/')[3];
        const body = await request.json();
...
async function runWebsiteDiagnostics(url) {
  ...
  try {
    ...
    diagnostics.checks.contentType = {
      status: contentType && contentType.includes('text/html') ? 'pass' : 'warning',
      value: contentType
    };
    ...
    if (response.ok) {
      ...
      diagnostics.checks.htmlStructure = {
        status: html.includes('<html') && html.includes('</html>') ? 'pass' : 'fail',
...
function generateRecommendations(diagnostics) {
  ...
  if (diagnostics.checks.performance?.responseTime > 5000) {
    recommendations.push({
      ...
      action: 'Optimize server performance, check CDN configuration, review asset sizes.'
    });
  }
