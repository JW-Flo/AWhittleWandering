...
describe('API Integration Tests', () => {
  ...
  describe('Performance Tests', () => {
    ...
    it('should handle burst traffic', async () => {
      ...
      const promises = Array(burstSize).fill().map((_, i) => 
        testRunner.makeRequest('/health').then(response => ({ index: i, ...response }))
      );
  ...
  describe('Data Consistency', () => {
    ...
    it('should show realistic data changes over time', async () => {
      ...
      const batteryDiff = Math.abs(initial.data.battery_level - later.data.battery_level);
  ...
  describe('Security Tests', () => {
    ...
    it('should handle SQL injection attempts safely', async () => {
      ...
      for (const input of maliciousInputs) {
        const response = await testRunner.makeRequest(`/api/tesla-data?param=${encodeURIComponent(input)}`);
