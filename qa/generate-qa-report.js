...
class QAReportGenerator {
    ...
    async generateComprehensiveReport() {
        ...
        const reportPath = path.join(this.reportsDir, `comprehensive-qa-report-${Date.now()}.json`);
    ...
    async generateSummary() {
        ...
        const totalDuration = recentReports.reduce((sum, r) => {
            ...
            return sum + Object.values(phases).reduce((pSum, p) => pSum + (p.duration || 0), 0);
        }, 0);
        ...
        return {
            ...
            status: successful.length > recentReports.length * 0.8 ? 'healthy' : 'concerning'
        };
    }
    ...
    async analyzeTrends() {
        ...
        const recentFailureRate = this.calculateAverageFailureRate(recentDays, dailyStats);
        const previousFailureRate = this.calculateAverageFailureRate(previousDays, dailyStats);
        ...
        return {
            ...
            failureRateTrend: {
                ...
                direction: recentFailureRate > previousFailureRate ? 'increasing' : 
                          recentFailureRate < previousFailureRate ? 'decreasing' : 'stable'
            },
    ...
    async generateRecommendations() {
        ...
        if (summary.averageDuration > 300000) { // 5 minutes
            recommendations.push({
                priority: 'medium',
                category: 'performance',
                title: 'Long Test Duration',
                description: `Average test duration is ${Math.round(summary.averageDuration / 1000)}s. Consider optimization.`,
                actions: [
        ...
        const topFailure = Object.entries(failures.phaseFailures)
            .sort(([,a], [,b]) => b - a)[0];
    ...
    async getRecentAlerts() {
        try {
            ...
            return alertFiles.map(file => {
                const content = fs.readFileSync(path.join(this.alertsDir, file), 'utf8');
    ...
    calculateTotalDuration(report) {
        ...
        return Object.values(phases).reduce((sum, phase) => sum + (phase.duration || 0), 0);
    }

    calculateAverageFailureRate(dates, dailyStats) {
        if (dates.length === 0) return 0;
        
        const totalRuns = dates.reduce((sum, date) => sum + (dailyStats[date]?.runs || 0), 0);
        const totalFailures = dates.reduce((sum, date) => sum + (dailyStats[date]?.failures || 0), 0);
    ...
    analyzePerformanceTrend(dailyStats, dates) {
        ...
        return {
            ...
            trend: recentAvg > previousAvg * 1.1 ? 'degrading' :
                   recentAvg < previousAvg * 0.9 ? 'improving' : 'stable'
        };
    }
    ...
    calculateAverageDuration(dates, dailyStats) {
        ...
        const totalDuration = dates.reduce((sum, date) => sum + (dailyStats[date]?.totalDuration || 0), 0);
        const totalRuns = dates.reduce((sum, date) => sum + (dailyStats[date]?.runs || 0), 0);
...
<head>
    ...
    <title>QA Report - ${new Date(report.metadata.generatedAt).toLocaleDateString()}</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p class="timestamp">Generated: ${new Date(report.metadata.generatedAt).toLocaleString()}</p>
        </div>
        ...
        <div class="section">
            ...
            <div class="summary">
                <div class="metric">
                    <div class="metric-value ${report.summary.successRate >= 80 ? 'success' : 'danger'}">${report.summary.successRate}%</div>
                ...
                <div class="metric">
                    <div class="metric-value ${report.summary.status === 'healthy' ? 'success' : 'warning'}">${report.summary.status}</div>
                ...
                <tbody>
                    ${report.recentRuns.map(run => `
                        <tr>
                            ...
                            <td class="${run.status === 'passed' ? 'success' : 'danger'}">${run.status}</td>
            ...
            <div class="failures">
                ...
                <ul>
                    ${Object.entries(report.failures.phaseFailures)
                        .sort(([,a], [,b]) => b - a)
                        .slice(0, 5)
                        .map(([phase, count]) => `<li>${phase}: ${count} failures</li>`)
                        .join('')}
                </ul>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <table>
                ...
                <tbody>
                    ${Object.entries(report.performance).map(([phase, metrics]) => `
                        <tr>
                            <td>${phase}</td>
