...
import { RefreshCw, Car, Zap, Route, Activity, MapPin, LogIn, UserPlus } from 'lucide-react';
...
const Index = () => {
  // Login/Register feedback state
  const [authMessage, setAuthMessage] = useState<string | null>(null);
  const [authLoading, setAuthLoading] = useState(false);

  // Background function for Login/Register
  const handleAuth = async (action: 'login' | 'register') => {
    ...
    try {
      ...
      setAuthMessage(data.message || `${action.charAt(0).toUpperCase() + action.slice(1)} successful!`);
    } catch (err) {
      setAuthMessage(err instanceof Error ? err.message : 'Auth error');
    } finally {
