...
class PerformanceMonitor {
   ...
   */
  trackMapMetrics(mapInstance: any): void {
   ...
   */
  trackApiRequest(url: string, startTime: number, success: boolean, responseSize?: number): void {
    const duration = performance.now() - startTime;
   ...
   */
  getPerformanceSummary(): WebVitalsData & { customMetrics: PerformanceMetric[] } {
    const webVitals: WebVitalsData = {};
    const customMetrics: PerformanceMetric[] = [];

    this.metrics.forEach(metric => {
      if (metric.type === 'web-vital') {
        (webVitals as any)[metric.name] = metric.value;
      } else {
  ...
  checkPerformanceTargets(): { 
    score: number; 
    passing: boolean; 
    details: Record<string, { value: number; target: number; passing: boolean }> 
  } {
    const targets = {
    ...
    const details: Record<string, { value: number; target: number; passing: boolean }> = {};
    let totalScore = 0;
...
export function usePerformanceMonitor() {
  ...
  const recordMetric = (metric: PerformanceMetric) => performanceMonitor.recordMetric(metric);
  ...
  return {
    ...
    checkPerformanceTargets,
    trackMapMetrics: (mapInstance: any) => performanceMonitor.trackMapMetrics(mapInstance),
    trackApiRequest: (url: string, startTime: number, success: boolean, responseSize?: number) => 
      performanceMonitor.trackApiRequest(url, startTime, success, responseSize)
  };
}
