...
const TimelineDataDisplay: React.FC<TimelineDataDisplayProps> = ({ tessieApiKey }) => {
  ...
  // Epic adventure timeline data
  const adventureTimeline: TimelineEntry[] = [
    { date: '🚀 June 1', state: 'Texas', keyStops: 'ADVENTURE BEGINS! Starting from Corpus Christi', type: 'milestone' },
    { date: 'June 2–3', state: 'New Mexico', keyStops: '🏜️ Carlsbad Caverns - Underground wonderland!', type: 'scenic' },
    { date: 'June 4', state: 'Texas', keyStops: '🛣️ Fort Stockton overnight - Desert highway vibes', type: 'adventure' },
    { date: 'June 5', state: 'Texas', keyStops: '🔧 El Paso Tesla service - Road trip preparation', type: 'adventure' },
    { date: 'June 6–7', state: 'Arizona', keyStops: '🌄 Sedona & Grand Canyon Desert View Watchtower!', type: 'scenic' },
    { date: 'June 8', state: 'Utah', keyStops: '⛰️ Zion National Park - First Utah conquest!', type: 'scenic' },
    { date: 'June 9', state: 'Nevada', keyStops: '🎰 Vegas drive-through - Desert oasis', type: 'cultural' },
    { date: 'June 9', state: 'California', keyStops: '🌴 Vegas → Los Angeles - Pacific dreams begin!', type: 'milestone' },
    { date: 'June 9–13', state: 'California', keyStops: '🏖️ 4 epic days in LA, then PCH northbound!', type: 'adventure' },
    ...
    { date: 'June 17', state: 'Washington', keyStops: '🌲 Sequim - Olympic Peninsula adventure', type: 'scenic' },
    { date: 'June 18', state: 'Washington', keyStops: '🏙️ Seattle with Brian - City exploration!', type: 'cultural' },
    { date: 'June 19', state: 'Washington', keyStops: '🥾 Quincy hike - Mountain adventures', type: 'adventure' },
    ...
    { date: 'June 25–26', state: 'Utah', keyStops: '🏔️ Salt Lake City → 2-day Provo adventure', type: 'adventure' },
    { date: 'June 27–28', state: 'Colorado', keyStops: '🏔️ Denver (Josh) & Fort Collins (Caleb) reunion!', type: 'cultural' },
    { date: 'July 3', state: 'Nebraska', keyStops: '🌾 Arrived Lincoln for epic 4-day stay', type: 'adventure' },
    { date: 'July 4', state: 'Iowa', keyStops: '🇺🇸 Council Bluffs 4th of July drive-through', type: 'cultural' },
    { date: 'July 4', state: 'South Dakota', keyStops: '🏜️ Council Bluffs → Badlands adventure!', type: 'scenic' },
    ...
    { date: 'July 14', state: 'New York', keyStops: '🏛️ Albany with Phil Dalton - Capital adventures', type: 'cultural' },
    ...
    { date: 'July 25', state: 'Rhode Island', keyStops: '🌊 Watch Hill Point coastal adventure!', type: 'scenic' }
  ];
  ...
  const getTypeColor = (type?: string) => {
    switch (type) {
      ...
      case 'scenic': return 'text-green-400 bg-green-400/20';
  ...
  return (
    <div className="space-y-8">
      {/* Epic Journey Overview */}
      <Card className="bg-gradient-to-br from-purple-900/30 via-blue-900/30 to-indigo-900/30 border-purple-500/20 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-3 text-3xl font-bold text-white">
            <Trophy className="w-8 h-8 text-yellow-400" />
            Epic Journey Overview
            <Badge className="bg-yellow-400/20 text-yellow-400 text-sm">LIVE ADVENTURE</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center p-4 rounded-xl bg-gradient-to-br from-green-600/20 to-green-800/20 border border-green-500/30">
              <div className="text-4xl font-bold text-green-400 mb-2">{statesVisited}</div>
              <div className="text-green-200 font-medium">States Conquered</div>
              <Badge className="mt-2 bg-green-400/20 text-green-400 text-xs">
                {48 - statesVisited} remaining
              </Badge>
            </div>
            
            <div className="text-center p-4 rounded-xl bg-gradient-to-br from-blue-600/20 to-blue-800/20 border border-blue-500/30">
              <div className="text-4xl font-bold text-blue-400 mb-2">{totalMiles.toLocaleString()}</div>
              <div className="text-blue-200 font-medium">Epic Miles</div>
              <Badge className="mt-2 bg-blue-400/20 text-blue-400 text-xs">
                {averageDailyMiles}/day avg
              </Badge>
            </div>
            
            <div className="text-center p-4 rounded-xl bg-gradient-to-br from-purple-600/20 to-purple-800/20 border border-purple-500/30">
              <div className="text-4xl font-bold text-purple-400 mb-2">{daysElapsed}</div>
              <div className="text-purple-200 font-medium">Days of Adventure</div>
              <Badge className="mt-2 bg-purple-400/20 text-purple-400 text-xs">
                since June 1st
              </Badge>
            </div>
            
            <div className="text-center p-4 rounded-xl bg-gradient-to-br from-orange-600/20 to-orange-800/20 border border-orange-500/30">
              <div className="text-4xl font-bold text-orange-400 mb-2">{progressPercentage.toFixed(1)}%</div>
              <div className="text-orange-200 font-medium">Complete</div>
              <Badge className="mt-2 bg-orange-400/20 text-orange-400 text-xs">
                Continental USA
              </Badge>
            </div>
          </div>
      ...
      {/* Adventure Timeline */}
      <Card className="bg-gradient-to-br from-indigo-900/30 via-purple-900/30 to-pink-900/30 border-indigo-500/20 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-3 text-3xl font-bold text-white">
            <Calendar className="w-8 h-8 text-indigo-400" />
            Adventure Timeline
            <Badge className="bg-indigo-400/20 text-indigo-400 text-sm">EVERY EPIC MOMENT</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            <p className="text-xl text-gray-300 text-center font-light">
              Every milestone, every state, every unforgettable moment of this Tesla road trip adventure! 🚗⚡
            </p>
            
            <div className="grid gap-4 max-h-96 overflow-y-auto pr-4 space-y-3">
              {adventureTimeline.map((entry, index) => {
                    ...
                    <div className="flex items-start gap-4 p-4 rounded-xl bg-gradient-to-r from-gray-800/50 to-gray-900/50 hover:from-gray-700/50 hover:to-gray-800/50 transition-all duration-300 border border-gray-700/50 hover:border-gray-600/50 backdrop-blur-sm">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <Badge className="bg-indigo-500/20 text-indigo-400 px-3 py-1 font-medium">
                            {entry.date}
                          </Badge>
                          <Badge className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-300 px-3 py-1 font-medium border border-purple-500/30">
                            {entry.state}
                          </Badge>
                          {entry.type && (
