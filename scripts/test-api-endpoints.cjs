...
async function testVehicleAPI() {
  ...
  try {
    ...
    log(
      ...
      responseTime < 2000 ? "green" : "yellow"
    );
    log(
    ...
    const missingFields = API_ENDPOINTS.vehicle.expectedFields.filter(
      (field) => !(field in data) && !(data.location && field in data.location)
    );
    ...
    log(
      `  Location: [${data.latitude || data.location?.latitude || "N/A"}, ${
        data.longitude || data.location?.longitude || "N/A"
      }]`
    );
...
async function testWeatherAPI() {
  ...
  try {
    ...
    if (response.status === 401) {
      log(
        "⚠️  API Key required - set OPENWEATHER_API_KEY environment variable",
        "yellow"
      );
      return {
        success: false,
...
async function testRoutingAPI() {
  ...
  try {
    ...
    log(
      `Response Time: ${responseTime}ms`,
      responseTime < 3000 ? "green" : "yellow"
    );
    log(
      `Status: ${response.status} ${response.statusText}`,
      response.ok ? "green" : "red"
    ...
    if (response.status === 401) {
      log(
        "⚠️  Mapbox token required - set MAPBOX_TOKEN environment variable",
        "yellow"
      );
      return {
        success: false,
    ...
    return {
      success: true,
    // Resolve data directory using environment variable or fallback to absolute path
    const dataDir = process.env.ITINERARY_DATA_DIR
      ? path.resolve(process.env.ITINERARY_DATA_DIR)
      : path.resolve(__dirname, '../../data');
    log(`Resolved itinerary data directory: ${dataDir}`, 'yellow');
  ...
  try {
    ...
    if (jsonFiles.length > 0) {
      ...
      if (Array.isArray(data)) {
        ...
        log(
          `  First stop: ${data[0]?.city || data[0]?.name}, ${data[0]?.state}`
        );
        log(
          `  Last stop: ${
            data[data.length - 1]?.city || data[data.length - 1]?.name
...
async function runAPITests() {
  ...
  if (!results.vehicle.success) {
    log(
      "  • Vehicle API: Check edge worker deployment and Tessie API credentials"
    );
  }
  ...
  if (!results.itinerary.success) {
    log(
      "  • Itinerary: Ensure data files are present in ../../data/ directory"
    );
  }
