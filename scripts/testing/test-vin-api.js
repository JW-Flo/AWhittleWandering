...
async function testVinBasedAPI() {
  try {
    ...
    const drivesResponse = await fetch(
      `https://api.tessie.com/${TESLA_VIN}/drives?limit=5&since=1746076800`,
      {
    ...
    const chargesResponse = await fetch(
      `https://api.tessie.com/${TESLA_VIN}/charges?limit=5&since=1746076800`,
      {
        headers: { Authorization: `Bearer ${TESSIE_API_TOKEN}` },
      }
    );
    ...
    const altDrivesResponse = await fetch(
      `https://api.tessie.com/drives?vin=${TESLA_VIN}&limit=5&since=1746076800`,
      {
        headers: { Authorization: `Bearer ${TESSIE_API_TOKEN}` },
      }
    );
