...
// Configuration
const CSV_FILE = path.join(__dirname, "..", "docs", "AWhittleWandering_Final_With_Deanna_And_BarHarbor.csv");
...
const cityCoordinates = {
  ...
  "Yellowstone NP, WY": { lat: 44.4280, lng: -110.5885 },
...
function determineStopType(notes) {
  ...
  if (lowerNotes.includes('visit') || lowerNotes.includes('stay') || lowerNotes.includes('camping')) return 'overnight';
  if (lowerNotes.includes('repair') || lowerNotes.includes('charging') || lowerNotes.includes('supercharger')) return 'charging';
  if (lowerNotes.includes('park') || lowerNotes.includes('hike') || lowerNotes.includes('scenic')) return 'attraction';
...
async function convertFinalItinerary() {
  try {
    console.log(`${BLUE}Converting final itinerary CSV to JSON format...${RESET}`);
    ...
    console.log(`${YELLOW}Processing ${records.length} itinerary records...${RESET}`);
    ...
    const stops = records.map((record, index) => {
      ...
      if (!cityCoordinates[city]) {
        console.warn(`${YELLOW}Warning: No coordinates found for ${city}${RESET}`);
      }
      ...
      features: [
        {
          ...
          properties: {
            ...
            description: "60-day Tesla road trip through all 48 contiguous US states",
            tripStart: "2025-06-06",
            tripEnd: "2025-08-09",
            totalStops: stops.length,
            totalMiles: stops.reduce((sum, stop) => sum + stop.milesFromPrev, 0),
            totalDriveHours: stops.reduce((sum, stop) => sum + stop.driveHours, 0),
            stops
          },
          geometry: {
            type: "LineString",
    ...
    console.log(`${BLUE}🛣️  Total miles: ${stops.reduce((sum, stop) => sum + stop.milesFromPrev, 0)}${RESET}`);
    ...
    const uploadCommand = `cd edge-worker && npx wrangler kv key put itinerary --binding=${KV_NAMESPACE} --path="${tempFile.replace(/\\/g, "/")}"`;
    ...
    console.log(`${GREEN}✅ Final itinerary uploaded to KV storage successfully!${RESET}`);
