...
class EndToEndTest {
  ...
  async testUserJourney() {
    ...
    await this.test('User lands on main site', async () => {
      ...
      if (!response.data.includes('<div id="root">') || !response.data.includes('A Whittle Wandering')) {
  ...
  async testDataConsistency() {
    ...
    await this.test('Trip data consistency', async () => {
      ...
      const statusResponse = await this.httpRequest(`${API_BASE_URL}/trip-status`);
      const unifiedResponse = await this.httpRequest(`${API_BASE_URL}/unified-data`);
  ...
  async testErrorScenarios() {
    ...
    await this.test('API rate limiting tolerance', async () => {
      ...
      const promises = Array(20).fill().map(() => 
        this.httpRequest(`${API_BASE_URL}/health`)
      );
    ...
    await this.test('Large data handling', async () => {
      ...
      if (dataSize > 50000) { // 50KB limit for reasonable response size
  ...
  async runFullE2ETest() {
    ...
    if (this.results.failed === 0) {
      this.log('\n🎉 ALL END-TO-END TESTS PASSED! User experience validated.', 'success');
    } else {
