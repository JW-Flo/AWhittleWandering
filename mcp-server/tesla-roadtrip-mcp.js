    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          description: 'Analyze driving efficiency by route, weather, or time period',
          inputSchema: {
            type: 'object',
            properties: {
              groupBy: { 
        ...
        },
        {
          ...
          description: 'Find charging stations near current location or specified coordinates',
          inputSchema: {
            type: 'object',
            properties: {
              latitude: { type: 'number', description: 'Latitude (uses current location if not provided)' },
              longitude: { type: 'number', description: 'Longitude (uses current location if not provided)' },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              departureTime: { type: 'string', description: 'Planned departure time (ISO format)' },
              maxChargeStops: { type: 'number', description: 'Maximum number of charging stops' }
            },
  ...
  async searchDrives(args) {
    ...
    if (args.state) {
      drives = drives.filter(d => 
        d.startLocation.includes(args.state) || d.endLocation.includes(args.state)
      );
    }
    if (args.minDistance) {
...
${drives.slice(0, 10).map(drive => 
  `• ${new Date(drive.date).toLocaleDateString()}: ${drive.startLocation} → ${drive.endLocation} (${drive.distance} mi)`
).join('\n')}
