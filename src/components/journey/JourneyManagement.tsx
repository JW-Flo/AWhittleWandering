...
export function JourneyManagement({ journey, onUpdate }: JourneyManagementProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${journey.name.replace(/\s+/g, '-').toLowerCase()}-export-${format(new Date(), 'yyyy-MM-dd')}.json`;
      ...
      toast({
        ...
        description: `Downloaded ${data.data.drives.length} drives and ${data.data.charges.length} charging sessions.`,
      });
    } catch (err: any) {
      toast({
        title: 'Export Failed',
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription className="flex items-center gap-2 mt-1">
              ...
              {journey.end_date && ` - ${format(new Date(journey.end_date), 'MMM d, yyyy')}`}
              {journey.total_miles && (
