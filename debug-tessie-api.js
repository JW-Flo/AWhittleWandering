  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(vehiclesData)) {
        ...
        console.log("📋 Unknown response structure:", Object.keys(vehiclesData));
      ...
      if (targetVehicle) {
        ...
        const displayName = targetVehicle.last_state?.vehicle_state?.vehicle_name || targetVehicle.last_state?.display_name;
        
        console.log(`\n🎯 TARGET VEHICLE FOUND:`);
        console.log("  - Vehicle ID:", vehicleId);
        console.log("  - VIN:", targetVehicle.vin);
        console.log("  - Name:", displayName);
        console.log("  - Battery Level:", targetVehicle.last_state?.charge_state?.battery_level + "%");
        console.log("  - Charging State:", targetVehicle.last_state?.charge_state?.charging_state);
        console.log("  - Location:", targetVehicle.last_state?.drive_state?.latitude, targetVehicle.last_state?.drive_state?.longitude);
        ...
        const drivesResponse = await fetch(`https://api.tessie.com/${vehicleId}/drives?limit=5&since=1746076800`, {
          headers: { 'Authorization': `Bearer ${TESSIE_API_TOKEN}` }
        });
        ...
        const chargesResponse = await fetch(`https://api.tessie.com/${vehicleId}/charges?limit=5&since=1746076800`, {
          headers: { 'Authorization': `Bearer ${TESSIE_API_TOKEN}` }
        });
        ...
        console.log("Available VINs:", vehicles.map(v => v.vin));
      }
