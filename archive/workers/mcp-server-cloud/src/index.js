...
class TeslaAPI {
  ...
  async fetchUnifiedData() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Tesla API error: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
  ...
  async fetchHealth() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Health check via unified data failed: ${response.status} ${response.statusText}`
        );
      }
      const data = await response.json();
...
const MCP_TOOLS = [
  ...
  },
  {
    name: "search_drives",
    description:
      "Search driving segments by date, location, or distance with advanced filtering",
    inputSchema: {
      type: "object",
      properties: {
        startDate: {
        ...
        state: {
          type: "string",
          description: "Filter by state abbreviation or full name",
        },
        minDistance: {
          type: "number",
  ...
  },
  {
    ...
    description:
      "Get system health status including API connectivity and data freshness",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      "Get current live location with detailed positioning information",
    inputSchema: {
      type: "object",
      properties: {
        includeNearby: {
          type: "boolean",
          description: "Include nearby points of interest",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Calculate predictions for trip completion, mileage targets, and state visits",
    inputSchema: {
      type: "object",
      properties: {
        targetDate: {
        ...
        includeAlternativeRoutes: {
          type: "boolean",
          description: "Include alternative route suggestions",
        },
      },
      required: [],
    },
...
- Coordinates: ${location.coordinates.lat.toFixed(
            4
...
- Last Update: ${new Date(data.tessieStatus.lastUpdate).toLocaleString()}`,
  ...
  async getTripOverview() {
    ...
    const dailyAverage = (overview.totalMiles / overview.daysElapsed).toFixed(
      0
    );
...
- ${
            overview.totalMiles > 10000
              ? "Major milestone: Over 10k miles!"
              : "Building toward the 10k mile mark"
          }`,
        },
      ],
    };
  ...
  async searchDrives(args) {
    ...
    const avgDistance =
      drives.length > 0 ? (totalDistance / drives.length).toFixed(1) : 0;
...
${
  drives.length > limit
    ? `\n💡 **Note:** ${
        drives.length - limit
      } additional drives match your criteria. Adjust the limit parameter to see more.`
    : ""
}`,
        },
      ],
...
- Charging Locations: ${
        new Set(charges.map((c) => c.location)).size
      } unique locations`;
    }
      ...
      content: [
        {
          ...
          text: `⚡ **Charging History** ${
            args.days ? `(Last ${args.days} days)` : "(All time)"
          }
...
${charges
  ...
  .map(
    (charge, i) =>
      `${i + 1}. **${new Date(charge.date).toLocaleDateString()}** - ${
        charge.location
      }
   ⚡ ${charge.energyAdded} kWh | 💰 $${charge.cost.toFixed(2)} | ⏰ ${
        charge.startTime
      } - ${charge.endTime}`
  )
  .join("\n\n")}${statisticsText}
    ...
    drives.forEach((drive) => {
      ...
      switch (args.groupBy) {
        case "state":
          ...
          break;
        case "month":
          groupKey = `${driveDate.getFullYear()}-${String(
            driveDate.getMonth() + 1
          ).padStart(2, "0")}`;
          break;
        case "week":
...
- Analysis Type: ${
            args.groupBy.charAt(0).toUpperCase() + args.groupBy.slice(1)
          }-based grouping
...
**💡 Insights:**
- Most active ${args.groupBy}: ${analysisResults[0]?.group} (${
            analysisResults[0]?.totalDistance
          } miles)
...
- Recommended Pace: ${
        feasible
          ...
          : "Increase by " +
            ((requiredStatesPerDay / statesPerDay - 1) * 100).toFixed(0) +
            "%"
      }`;
    }
...
- Final Projected Mileage: ${(
            overview.totalMiles +
            estimatedDaysToComplete * milesPerDay
          ).toLocaleString()}

**📈 Milestone Predictions:**
- Next 1,000 miles: ${Math.ceil(
            (Math.ceil(overview.totalMiles / 1000) * 1000 -
              overview.totalMiles) /
              milesPerDay
          )} days
- Next 5 states: ${Math.ceil(5 / statesPerDay)} days
- Halfway point: ${
            overview.statesVisited >= 24
              ? "✅ Already passed!"
              : Math.ceil((24 - overview.statesVisited) / statesPerDay) +
                " days"
          }${targetAnalysis}
...
${
  args.includeAlternativeRoutes
    ? `
- Current efficiency: ${(overview.totalMiles / overview.statesVisited).toFixed(
        0
      )} miles per state
- Optimal route suggestions available for remaining states
- Consider clustering nearby states for efficiency`
    : "Enable alternative routes analysis for detailed suggestions"
}
  ...
  try {
    ...
    switch (request.method) {
      ...
      case "tools/call":
        const teslaAPI = new TeslaAPI(c.env);
        const handler = new MCPToolHandler(teslaAPI);
        const result = await handler.handleTool(
          request.params.name,
