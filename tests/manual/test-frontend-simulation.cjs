...
class FrontendSimulation {
  ...
  async testAPIConnectivity() {
    ...
    try {
      ...
      const response = await fetchImpl(`${this.config.apiUrl}/unified-data`, {
        method: "GET",
        headers: {
      ...
      if (!data.overview || !data.currentStatus) {
        this.log(
          "⚠️  API response missing expected structure, but connection works",
          "warn"
        );
  ...
  async testFrontendAssets() {
    ...
    try {
      // Test main HTML
      const htmlResponse = await fetchImpl(`${this.config.frontendUrl}/`, {
        headers: {
          "User-Agent": this.config.userAgent,
        },
      });
  ...
  async testCORSConfiguration() {
    ...
    try {
      const response = await fetchImpl(`${this.config.apiUrl}/health`, {
        method: "OPTIONS",
        headers: {
          Origin: this.config.frontendUrl,
  ...
  async runSimulation() {
    ...
    this.log(
      `🔧 Fetch Implementation: ${fetchImpl.name || "native fetch"}`,
      "info"
    );
    this.log("=".repeat(50), "info");
