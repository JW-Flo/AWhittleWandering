...
async function testFrontendLogic() {
  ...
  const calculateJourneyStats = () => {
    ...
    if (mockData?.timeline?.drives && mockData.timeline.drives.length > 0) {
      const sortedDrives = [...mockData.timeline.drives].sort(
  ...
  const getCurrentLocation = () => {
    if (
      mockData?.currentStatus?.location?.city &&
      mockData?.currentStatus?.location?.state
    ) {
      return {
  ...
  const calculateTotalMiles = () => {
    ...
    const driveMiles = mockData.timeline.drives.reduce(
      (total, drive) => total + (drive.distance || 0),
      0
    );
    return driveMiles;
  };
  ...
  console.log("\n6. DATE PARSING TEST");
  console.log(
    `Journey Start: ${journeyStats.earliestDrive?.toLocaleDateString()}`
  );
  console.log(`Journey End: ${journeyStats.latestDrive?.toLocaleDateString()}`);
  console.log(
    journeyStats.earliestDrive && journeyStats.latestDrive
      ? "✅ PASS"
      : "❌ FAIL"
  );
