...
export const reportTokenStatus = () => {
  ...
  const windowToken = typeof window !== 'undefined' ? window.__MAPBOX_TOKEN__ : null;
  ...
  // Identify issues
  const issues = [];
  if (!diagnostics.configuredToken.exists) issues.push('Token not available from environmentConfig');
  if (!diagnostics.mapboxGl.exists) issues.push('mapboxgl.accessToken not set');
  if (!diagnostics.envVariable.exists) issues.push('VITE_MAPBOX_TOKEN env variable not injected');
