...
class TeslaDataIngestion {
  ...
  async ingestDrivesData() {
    ...
    console.log(
      `✅ Drives ingestion complete: ${this.totalDrives} records processed`
    );
  }
  ...
  async processDrivesBatch(drives, startIndex) {
    ...
    process.stdout.write(
      `  Batch ${batchNumber}: Processing ${drives.length} drives... `
    );

    const processedDrives = drives.map((drive) =>
      this.mapDriveToD1Schema(drive)
    );
  ...
  mapDriveToD1Schema(drive) {
    return {
      ...
      efficiency_mi_per_kwh:
        drive.energy_used > 0 ? drive.odometer_distance / drive.energy_used : 0,
  ...
  async ingestChargesData() {
    ...
    console.log(
      `✅ Charges ingestion complete: ${this.totalCharges} records processed`
    );
  }
  ...
  async processChargesBatch(charges, startIndex) {
    const batchNumber = Math.floor(startIndex / this.batchSize) + 1;
    process.stdout.write(
      `  Batch ${batchNumber}: Processing ${charges.length} charges... `
    );

    const processedCharges = charges.map((charge) =>
      this.mapChargeToD1Schema(charge)
    );
  ...
  mapChargeToD1Schema(charge) {
    return {
      ...
      cost_per_kwh:
        charge.energy_added > 0 ? charge.cost / charge.energy_added : 0,
      kwh_per_minute:
        charge.duration_minutes > 0
          ? charge.energy_added / charge.duration_minutes
          : 0,
  ...
  async updateVehicleState() {
    ...
    console.log(
      `  Battery: ${vehicleStateRecord.battery_level}% (${vehicleStateRecord.charging_state})`
    );
    console.log(
      `  Location: ${vehicleStateRecord.latitude}, ${vehicleStateRecord.longitude}`
    );
  ...
  generateIngestionSummary() {
    ...
    console.log(
      "🔄 Frontend components can now access comprehensive Tesla data"
    );
